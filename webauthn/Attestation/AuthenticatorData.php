...
class moWebAuthn_AuthenticatorData {
    protected $_binary;
    protected $_rpIdHash;
    protected $_flags;
    ...
    private function _readCredentialPublicKey($binary, $offset, &$endOffset) {
        $enc = moWebAuthn_CborDecoder::decodeInPlace($binary, $offset, $endOffset);
    ...
    private function _readCredentialPublicKeyES256(&$credPKey, $enc) {
        ...
        $credPKey->x   = $enc[self::$_COSE_X] instanceof moWebAuthn_ByteBuffer ? $enc[self::$_COSE_X]->getBinaryString() : null;
        $credPKey->y   = $enc[self::$_COSE_Y] instanceof moWebAuthn_ByteBuffer ? $enc[self::$_COSE_Y]->getBinaryString() : null;
        unset ($enc);
    ...
    private function _readCredentialPublicKeyRS256(&$credPKey, $enc) {
        $credPKey->n = $enc[self::$_COSE_N] instanceof moWebAuthn_ByteBuffer ? $enc[self::$_COSE_N]->getBinaryString() : null;
        $credPKey->e = $enc[self::$_COSE_E] instanceof moWebAuthn_ByteBuffer ? $enc[self::$_COSE_E]->getBinaryString() : null;
        unset ($enc);
    ...
    private function _readExtensionData($binary) {
        $ext = moWebAuthn_CborDecoder::decode($binary);
        if (!\is_array($ext)) {
