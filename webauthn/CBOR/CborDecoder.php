...
class moWebAuthn_CborDecoder {
    ...
    public static function decode($bufOrBin) {
        $buf = $bufOrBin instanceof moWebAuthn_ByteBuffer ? $bufOrBin : new moWebAuthn_ByteBuffer($bufOrBin);
     ...
     * @return mixed
     */
    protected static function _parseItem(moWebAuthn_ByteBuffer $buf, &$offset) {
    ...
    protected static function _parseFloatSimple($val, moWebAuthn_ByteBuffer $buf, &$offset) {
        switch ($val) {
            case 24:
    ...
    protected static function _parseExtraLength($val, moWebAuthn_ByteBuffer $buf, &$offset) {
        switch ($val) {
            case 24:
                $val = $buf->getByteVal($offset);
                $offset++;
    ...
    protected static function _parseItemData($type, $val, moWebAuthn_ByteBuffer $buf, &$offset) {
        switch ($type) {
            ...
            case self::CBOR_MAJOR_TAG:
                return self::_parseItem($buf, $offset); 
        }

        throw new WebAuthnException(sprintf('Unknown major type %d.', $type), WebAuthnException::CBOR);
    }

    protected static function _parseMap(moWebAuthn_ByteBuffer $buf, &$offset, $count) {
        $map = array();
    ...
    protected static function _parseArray(moWebAuthn_ByteBuffer $buf, &$offset, $count) {
        $arr = array();
        for ($i = 0; $i < $count; $i++) {
